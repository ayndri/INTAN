/******/ (function () {
  ...
  /*!*************************************************!*\
  !*** ./resources/js/category-management.js ***!
  \*************************************************/
  /**
   * Page Category List
   */
  ...
  $(document).on('input', '#add-category-name', function () {
    // Get the current value of the category_name input field
    var categoryName = $(this).val();

    // Convert category name to Title Case (First letter of each word capitalized)
    var titleCaseCategoryName = categoryName.toLowerCase().replace(/\b\w/g, function (char) {
      return char.toUpperCase();
    });

    // Update the input field value with the Title Case version
    $(this).val(titleCaseCategoryName);
  });

  document.getElementById('add-category-name').addEventListener('change', function () {
    let categoryName = this.value;
    let slug = categoryName.toLowerCase().replace(/\s+/g, '-');
    document.getElementById('add-category-slug').value = slug;
  });
  ...
  $(function () {
    ...
    var dt_user_table = $('.datatables-categories'),
      select2 = $('.select2'),
      ...
      var dt_user = dt_user_table.DataTable({
        ...
        columnDefs: [
          ...
          },
          {
            ...
            render: function render(data, type, full, meta) {
              var $slug = full['slug'];
              return '<span class="user-email">' + $slug + '</span>';
            }
          },
          {
            // User full name
            ...
            render: function render(data, type, full, meta) {
              return (
                ...
                '<button class="btn btn-sm btn-icon edit-record" data-id="'.concat(
                  full['id'],
                  '" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddCategory"><i class="ti ti-edit"></i></button>'
                ) +
    ...
    $(document).on('click', '.delete-record', function () {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            type: 'GET',
            url: ''.concat(baseUrl, 'categories/').concat(category_id, '/delete'),
    ...
    $(document).on('click', '.edit-record', function () {
      ...
      // get data
      $.get(''.concat(baseUrl, 'categories/').concat(category_id, '/edit'), function (data) {
        $('#category_id').val(data.id);
        $('#add-category-name').val(data.name); // Set category name field
        $('#add-category-slug').val(data.slug); // Set description field
        // Ensure that status is set properly (1 or 0)

        if (data.status !== undefined && (data.status === true || data.status === false)) {
          $('#add-category-status').prop('checked', false); // Uncheck first to reset

          if (data.status === true) {
            $('#add-category-status').prop('checked', true); // Set checked if active (1)
          } else if (data.status === 0) {
            $('#add-category-status').prop('checked', false); // Set unchecked if inactive (0)
          }
        }
      });
    });
    ...
    var addNewCategoryForm = document.getElementById('addNewCategoryForm');
    ...
    var fv = FormValidation.formValidation(addNewCategoryForm, {
      fields: {
        category_name: {
          validators: {
            notEmpty: {
            ...
            stringLength: {
              max: 100,
              message: 'The category name must be less than 100 characters'
            }
          }
        },
        description: {
      ...
      // Get the value of the category_name field
      ...
      // Update the value of the category_name field with the Title Case version
      $('#add-category-name').val(titleCaseCategoryName);

      $.ajax({
        data: $('#addNewCategoryForm').serialize(),
        url: ''.concat(baseUrl, 'categories/store'),
        type: 'POST',
        success: function success(status) {
          ...
          Swal.fire({
            ...
            text: 'Category '.concat(status, ' Successfully.'),
            customClass: {
              confirmButton: 'btn btn-success'
            }
          });
