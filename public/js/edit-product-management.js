/******/ (function () {
  ...
  document.getElementById('generate-sku-btn').addEventListener('click', function () {
    fetch('/product/generate-sku')
      .then(response => response.json())
      .then(data => {
        document.getElementById('add-product-sku').value = data.sku;
      })
      .catch(error => console.error('Error generating SKU:', error));
  });

  document.getElementById('generate-item-code-btn').addEventListener('click', function () {
    fetch('/product/generate-item-code')
      .then(response => response.json())
      .then(data => {
        document.getElementById('add-product-item-code').value = data.item_code;
      })
      .catch(error => console.error('Error generating Item Code:', error));
  });
  ...
  $(function () {
    ...
    document.getElementById('submitProductButton').onclick = function (e) {
      e.preventDefault(); // Mencegah submit default

      // Collect form data
      const formData = new FormData();
      formData.append('id', document.getElementById('add-product-id').value);
      formData.append('name', document.getElementById('add-product-name').value);
      formData.append('sku', document.getElementById('add-product-sku').value);
      formData.append('category_id', document.getElementById('add-product-category').value);
      formData.append('brand_id', document.getElementById('add-product-brand').value);
      formData.append('unit_id', document.getElementById('add-product-unit').value);
      formData.append('item_code', document.getElementById('add-product-item-code').value);
      formData.append('description', document.getElementById('add-product-description').value);
      formData.append('product_type', document.querySelector('input[name="productType"]:checked').value);
      formData.append('quantity', document.getElementById('add-product-quantity').value);
      formData.append('sell_price', document.getElementById('add-product-price').value.replace(/\./g, ''));
      formData.append('quantity_alert', document.getElementById('add-product-quantity-alert').value);

      // Collect variant options (if variable product is selected)
      const variantOptions = [];
      document.querySelectorAll('input[name="variant-options[]"]').forEach(input => {
        variantOptions.push(input.value);
      });
      formData.append('variantOptions', JSON.stringify(variantOptions));

      const dropzone = Dropzone.forElement('#dropzone-multi');
      ...
      $.ajax({
        url: `/product/${formData.get('id')}/edit`, // Gunakan ID produk dalam URL
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        }, // Jangan set contentType agar otomatis
        success: function (response) {
          Swal.fire({
            icon: 'success',
            title: `Produk berhasil ${response.toLowerCase()}!`,
            text: `Produk Anda telah ${response.toLowerCase()}.`,
            customClass: { confirmButton: 'btn btn-success' }
          }).then(() => {
            // Redirect to product page
            window.location.href = 'http://intan-web.test/product';
          });

          // Reset form dan preview gambar (optional, as the page will redirect)
          form.reset();
          document.getElementById('product-image-preview').src = 'https://via.placeholder.com/640x480.png?text=Preview';
        },
        error: function (err) {
          Swal.fire({
            icon: 'error',
            title: 'Error!',
            text: err.responseJSON?.message || 'Terjadi kesalahan saat menyimpan produk.',
