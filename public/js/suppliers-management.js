/******/ (function () {
  ...
  $(document).on('input', '#add-supplier-name', function () {
    ...
    $(this).val(titleCaseCustomerName);
  });
  ...
  $(function () {
    ...
    var dt_user_table = $('.datatables-suppliers'),
      ...
      offCanvasForm = $('#offcanvasAddCustomer');
    if (select2.length) {
      var $this = select2;
    ...
    if (dt_user_table.length) {
      var dt_user = dt_user_table.DataTable({
        ...
        columns: [
          ...
          {
            data: 'phone' // Supplier Stock (replacing 'quantity')
          },
          {
    ...
    $(document).on('click', '.delete-record', function () {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            type: 'GET',
            url: ''.concat(baseUrl, 'suppliers/').concat(customer_id, '/delete'),
    ...
    $(document).on('click', '.edit-record', function () {
      ...
      $('#offcanvasAddCustomerLabel').html('Edit Supplier');

      // get data
      $.get(''.concat(baseUrl, 'suppliers/').concat(customer_id, '/edit'), function (data) {
        $('#customer_id').val(data.id);
        $('#add-supplier-name').val(data.name); // Set supplier name field
        $('#add-supplier-email').val(data.email); // Set email field
        $('#add-supplier-phone').val(data.phone); // Set phone field
      });
    });
    ...
    $('.add-new').on('click', function () {
      ...
      $('#offcanvasAddCustomerLabel').html('Add Supplier');
    });
    ...
    var addNewCustomerForm = document.getElementById('addNewCustomerForm');
    ...
    var fv = FormValidation.formValidation(addNewCustomerForm, {
      ...
      var customerName = $('#add-supplier-name').val();
      ...
      $('#add-supplier-name').val(titleCaseCustomerName);

      $.ajax({
        data: $('#addNewCustomerForm').serialize(),
        url: ''.concat(baseUrl, 'suppliers/store'),
        type: 'POST',
