/******/ (function () {
  ...
  /*!*************************************************!*\
  !*** ./resources/js/supplier-management.js ***!
  \*************************************************/
  /**
   * Page Supplier List
   */
  ...
  $(document).on('input', '#add-supplier-name', function () {
    // Get the current value of the supplier_name input field
    var supplierName = $(this).val();

    // Convert supplier name to Title Case (First letter of each word capitalized)
    var titleCaseSupplierName = supplierName.toLowerCase().replace(/\b\w/g, function (char) {
      return char.toUpperCase();
    });

    // Update the input field value with the Title Case version
    $(this).val(titleCaseSupplierName);
  });
  ...
  $(function () {
    ...
    var dt_user_table = $('.datatables-suppliers'),
      select2 = $('.select2'),
      userView = baseUrl + 'app/user/view/account',
      offCanvasForm = $('#offcanvasAddSupplier');
    if (select2.length) {
      var $this = select2;
    ...
    if (dt_user_table.length) {
      var dt_user = dt_user_table.DataTable({
        ...
        ajax: {
          ...
          data: function (d) {
            d.status_filter = $('#UserStatus').val();
          }
        },
        columns: [
            ...
            render: function render(data, type, full, meta) {
              return (
                ...
                '<button class="btn btn-sm btn-icon edit-record" data-id="'.concat(
                  full['id'],
                  '" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddSupplier"><i class="ti ti-edit"></i></button>'
                ) +
    ...
    $(document).on('click', '.delete-record', function () {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            type: 'GET',
            url: ''.concat(baseUrl, 'suppliers/').concat(supplier_id, '/delete'),
    ...
    $(document).on('click', '.edit-record', function () {
      ...
      $('#offcanvasAddSupplierLabel').html('Edit Supplier');

      // get data
      $.get(''.concat(baseUrl, 'suppliers/').concat(supplier_id, '/edit'), function (data) {
        // Populate the supplier ID and supplier name
        $('#supplier_id').val(data.id);
        $('#add-supplier-name').val(data.name); // Set supplier name field

        // Set the supplier email
        $('#add-supplier-email').val(data.email);
        ...
        // Display the supplier's avatar image
        if (data.avatar) {
          var imageUrl;
    ...
    $('.add-new').on('click', function () {
      $('#supplier_id').val(''); //reseting input field
      $('#offcanvasAddSupplierLabel').html('Add Supplier');
      $('#addNewSupplierForm')[0].reset();
    ...
    var addNewSupplierForm = document.getElementById('addNewSupplierForm');

    // Ambil nilai form dan ubah nilai price dan cost sebelum di-serialize
    $('#addNewSupplierForm').submit(function (e) {
    ...
    var fv = FormValidation.formValidation(addNewSupplierForm, {
      fields: {
        supplier_name: {
          validators: {
            notEmpty: {
            ...
            stringLength: {
              max: 100,
              message: 'The supplier name must be less than 100 characters'
            }
          }
        }
      },
      ...
      // Get the value of the supplier_name field
      var supplierName = $('#add-supplier-name').val();
      var supplierStatus = $('#add-supplier-status').val(); // Check if the checkbox is checked
      ...
      $('#add-supplier-name').val(titleCaseSupplierName); // Update the supplier name with Title Case

      // Create a new FormData object using the form
      var formData = new FormData($('#addNewSupplierForm')[0]);

      // Include the supplier image in the form data if a file is selected
      var supplierImage = $('#supplier-image')[0].files[0];
      if (supplierImage) {
        formData.append('supplier_image', supplierImage);
      }

      // Make the AJAX request to submit the form data
      $.ajax({
        data: formData,
        url: ''.concat(baseUrl, 'suppliers/store'), // Your server URL
        type: 'POST',
        ...
        success: function (status) {
          ...
          Swal.fire({
            ...
            text: 'Supplier '.concat(status, ' successfully.'),
            customClass: {
              confirmButton: 'btn btn-success'
            }
          });
