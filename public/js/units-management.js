/******/ (function () {
  ...
  /*!*************************************************!*\
  !*** ./resources/js/unit-management.js ***!
  \*************************************************/
  /**
   * Page Unit List
   */
  ...
  $(document).on('input', '#add-unit-name', function () {
    // Get the current value of the unit_name input field
    var unitName = $(this).val();

    // Convert unit name to Title Case (First letter of each word capitalized)
    var titleCaseUnitName = unitName.toLowerCase().replace(/\b\w/g, function (char) {
  ...
  $(function () {
    ...
    var dt_user_table = $('.datatables-units'),
      select2 = $('.select2'),
    ...
    if (dt_user_table.length) {
      var dt_user = dt_user_table.DataTable({
        ...
        columnDefs: [
          ...
          },
          {
            ...
            render: function render(data, type, full, meta) {
              ...
              return '<span class="user-email">' + $unit_name + '</span>';
            }
          },
          {
            // User full name
            ...
            render: function render(data, type, full, meta) {
              return (
                ...
                '<button class="btn btn-sm btn-icon edit-record" data-id="'.concat(
                  full['id'],
                  '" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddUnit"><i class="ti ti-edit"></i></button>'
                ) +
        ...
        buttons: [
          ...
          {
            text: '<i class="ti ti-plus me-0 me-sm-1"></i><span class="d-none d-sm-inline-block">Add New Unit</span>',
    ...
    $(document).on('click', '.delete-record', function () {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            type: 'GET',
            url: ''.concat(baseUrl, 'units/').concat(unit_id, '/delete'),
    ...
    var addNewUnitForm = document.getElementById('addNewUnitForm');

    // Ambil nilai form dan ubah nilai price dan cost sebelum di-serialize
    $('#addNewUnitForm').submit(function (e) {
    ...
    var fv = FormValidation.formValidation(addNewUnitForm, {
      fields: {
        unit_name: {
          validators: {
            notEmpty: {
            ...
            stringLength: {
              max: 100,
              message: 'The unit name must be less than 100 characters'
            }
          }
        },
        description: {
      ...
      // Get the value of the unit_name field
      var unitName = $('#add-unit-name').val();
      ...
      // Update the value of the unit_name field with the Title Case version
      $('#add-unit-name').val(titleCaseUnitName);

      $.ajax({
        data: $('#addNewUnitForm').serialize(),
        url: ''.concat(baseUrl, 'units/store'),
        type: 'POST',
        success: function success(status) {
          ...
          Swal.fire({
            ...
            text: 'Unit '.concat(status, ' Successfully.'),
            customClass: {
              confirmButton: 'btn btn-success'
            }
          });
